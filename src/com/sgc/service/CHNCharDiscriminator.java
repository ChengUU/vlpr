 ...
 */
public class CHNCharDiscriminator extends Discriminator {
    private static volatile Discriminator discriminator=null;

    private CHNCharDiscriminator(RecogTemplate[] recgTemplates) {
        super(recgTemplates);
    }
    ...
    public static Discriminator getInstance(){
        ...
        if(null==discriminator){
            ...
            synchronized (Discriminator.class){
                // 未初始化则进行初始化
                RecogTemplate[] recgTemplates=getRecgTemplates(1);
                if(null==discriminator)
                    discriminator=new CHNCharDiscriminator(recgTemplates);
            }
        }
